...
2. `Store` connects to MongoDB, seeds the study once, and ensures the single administrator account exists.
...
8. A review or CAPA form posts JSON with the session CSRF token. The server checks the administrator session and current finding evidence, validates the request, saves it and appends an audit event.
